...
# ======================== SECCIÓN 1: Mapa de Avistajes ========================
if seleccion == "Mapa de Avistajes":
    st.title("Orcas Península Valdés")

    # Convertir latitud y longitud a numéricos y limpiar nulos
    df["Latitud"] = pd.to_numeric(df["Latitud"], errors="coerce")
    df["Longitud"] = pd.to_numeric(df["Longitud"], errors="coerce")
    df.dropna(subset=["Latitud", "Longitud"], inplace=True)

    # Crear dataframe de avistajes únicos
    ...
    # Filtrar avistajes que contengan alguna orca válida
    ...
    icono_fijo = CustomIcon(
        icon_image='Orca.png',  # Asegurate de tener el PNG en la carpeta correcta
        icon_size=(40, 40),
        icon_anchor=(20, 20)
    )
...
# ===================== SECCIÓN 2: Gráfico por Categoría ======================
...
# =================== SECCIÓN 3: Estadísticas Generales ===================
    ...
    # Orcas únicas con todos los filtros aplicados
    df_orcas_filtradas = df_filtrado.drop_duplicates(subset=["Orca"])

    # Orcas únicas SIN aplicar filtros de sexo/varadora (pero con otros filtros)
    df_base = df.copy()
    if grupos:
    ...
    # Ahora aplicamos filtros de sexo y varadora para que los gráficos y textos los usen
    if sexos:
